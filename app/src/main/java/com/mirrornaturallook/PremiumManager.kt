...
/**
 * Manages the 30-day free trial and $5 lifetime premium state.
 * Uses EncryptedSharedPreferences so purchase state is tamper-resistant.
 * On Google Play, the source of truth is Play Billing — this is the local cache.
 */
class PremiumManager(context: Context) {
    ...
    companion object {
        ...
        const val PRODUCT_ID_LIFETIME = "mirror_lifetime_premium" // Must match Play Console
    }
    ...
    init {
        // Record install date only once on first launch
    ...
    val installDateMs: Long
        get() = prefs.getLong(KEY_INSTALL_DATE, System.currentTimeMillis())

    val daysSinceInstall: Int
        get() {
            val diff = System.currentTimeMillis() - installDateMs
            return (diff / (1000 * 60 * 60 * 24)).toInt()
        }

    val trialDaysLeft: Int
        get() = maxOf(0, TRIAL_DAYS - daysSinceInstall)

    val isTrialExpired: Boolean
        get() = daysSinceInstall >= TRIAL_DAYS

    val needsPaywall: Boolean
        get() = !isPremium && isTrialExpired
    ...
    fun restorePremium(): Boolean {
        ...
        return premium && !token.isNullOrEmpty()
    }
